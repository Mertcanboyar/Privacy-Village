...
- **Multiplayer: DONE-MULTIPLAYER (local-only, scoped).** Live presence — other players visible, moving, named, faction-colored, in Village and Tavern — wired in and verified against the local dev server (two real browser tabs: mutual visibility, movement/facing sync, faction-colored tags, clean scene-transition despawn/respawn twice each direction, overlay-open standing pose, silent solo fallback with server down). Root cause of the old "first connection never receives other players" bug found and isolated: `getStateCallbacks`'s `onAdd`/`onChange` never fire in the Vite/browser bundle even though the raw schema state (`room.state.players.size`/`.forEach`) syncs correctly — proven with a standalone Node script using the same colyseus.js version, where the callbacks fire fine. `client/src/net/NetClient.ts` works around it by polling `room.state.players` once a frame instead of relying on those callbacks; see the file's header comment. **Deployment: server on Render, client on Vercel** — see [DEPLOY.md](DEPLOY.md) for the click-by-click setup. `client/src/net/NetClient.ts` reads the WS endpoint from `VITE_WS_URL` (client env var, `ws://localhost:2567` fallback); the server reads allowed CORS origins from `ALLOWED_ORIGINS` (comma-separated, `http://localhost:5173` fallback) and binds to `process.env.PORT`. `render.yaml` at the repo root is a Render Blueprint for the server.
